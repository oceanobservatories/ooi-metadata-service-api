############################################################################
# This is an integration test that needs uFrame running with empty         #
...
import requests
import unittest
# Equivalent to 'import api' but can find the 'api.py' file even with the '__init__.py' file next to it.
...
metadata_service_api = api.MetadataServiceAPI(STREAM_METADATA_SERVICE_URL, PARTITION_METADATA_SERVICE_URL)
...
class MetadataServiceTest(unittest.TestCase):
    ...
    def test_delete_stream_metadata_records(self):
        ...
        def check_was_deleted(id):
            with self.assertRaises(api.MetadataServiceException) as cm:
                url = '/'.join((STREAM_METADATA_SERVICE_URL, str(id)))
                response = metadata_service_api._MetadataServiceAPI__get_json(url)
    ...
    def test_delete_partition_metadata_records(self):
        ...
        # 3 good ones
        _, _, response4 = self.__partition_test_setup(
            1, 'test_store1', 1.1, 2.2, 3, method_suffix='0', stream_suffix='2'
        )
        _, _, response5 = self.__partition_test_setup(
            2, 'test_store2', 2.2, 3.3, 4, method_suffix='1', stream_suffix='1'
        )
        _, _, response6 = self.__partition_test_setup(
            3, 'test_store3', 3.3, 4.4, 5, method_suffix='2', stream_suffix='0'
        )
